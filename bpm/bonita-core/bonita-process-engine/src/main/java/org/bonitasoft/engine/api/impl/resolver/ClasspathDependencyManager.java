 ...
 */
public class ClasspathDependencyManager implements BusinessArchiveDependencyManager {

    private final DependencyService dependencyService;

    public ClasspathDependencyManager(DependencyService dependencyService) {
    ...
    private List<Long> getDependencyMappingsOfProcess(final DependencyService dependencyService, final long processDefinitionId) throws SDependencyException {
        ...
        do {
            currentPage = dependencyService.getDependencyIds(processDefinitionId, ScopeType.PROCESS, fromIndex, 100);
            dependencyIds.addAll(currentPage);
            fromIndex = fromIndex + 100;
        } while (currentPage.size() == 100);
